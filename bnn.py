...
Modified by Andrew Polar for verification of the accuracy of posterior 
distribution in the following way:
1. The actual normal distribution of target is changed into multimodal.
2. The posterior distribution built by MCMC is output at selected point.
"""
...
def get_data(N=50, D_X=3, sigma_obs=0.05, N_test=500):
    ...
    #First block added by Andrew Polar: it converts unimodal distribution of the target
    #into multimodal
    Z = np.random.randn(N)
    for i in range(N):
        if Z[i] > 0.0: Z[i] = 0.3
...
def main(args):
    ...
    #Second block added by Andrew Polar: it makes histogram of posterior distribution of 
    #the target at the point assigned below of X_test array. The size of X_test array
    #and interval is assigned at get_data.
    point = 150
    bins = 20
    histogram = np.histogram(predictions[:, point], bins)
    print(f'The size of MCMC sample {len(list(predictions[:, point]))}')
    ...
    x2 = []
    for i in range(bins):
        x2.append((x[i] + x[i + 1])/2.0)
    axs.plot(x2, y, "blue", ls="solid", lw=3.0)
    axs.set(xlabel="Target range", ylabel="Frequencies", 
        title=f"Posterior distribution of the target at the point x = {point} of input interval")
